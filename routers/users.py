...
async def forgot_password(
    ...
    if user:
        ...
        background_tasks.add_task(
            ...
            to_email=user.email,
            username=user.username,
            token=token,  # unhashed. User needs the unhashed token to reset password
        )
...
async def update_user(
    ...
    if (
        ...
        and user.username.lower() != user_update.username.lower()
    ):
...
async def upload_profile_picture(
    user_id: int,
    file: UploadFile,  # For multipart file uploads, which is fastAPI's way of sending file data
    current_user: CurrentUser,
    db: Annotated[AsyncSession, Depends(get_db)],
):
    ...
    if old_filename:
        await delete_profile_image(old_filename)  # Only deleting after a successful commit
